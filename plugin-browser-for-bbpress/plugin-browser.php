...
function plugin_browser_admin_page() {
	...
	$plugin_count_all       = 0;
	$plugin_count_active    = 0;
	$plugin_count_inactive  = 0;
	$plugin_count_installed = 0;
	$plugin_count_updates   = 0;
	
	foreach ( $plugins as $plugin_folder => $plugin ) {
		if ( $plugin['version'] ) {
			$plugin_count_all++;

			$cpf  = 'user#' . $plugin_folder . DIRECTORY_SEPARATOR . $plugin['file'];
			$cpf2 = 'user#pb--' . $plugin_folder . DIRECTORY_SEPARATOR . $plugin['file']; /* If the plugin had been installed earlier with this plugin */
			...
			if ( $bb_plugins[$cpf] || $bb_plugins[$cpf2] ) {
				...
				if ( version_compare( $plugin['version'], $bb_plugins[$cpf]['version'], '>' ) )
					$plugin_count_updates++;
				
				if ( in_array( $cpf, array_values( $active_plugins ) ) )
					$plugin_count_active++;
				else
					$plugin_count_inactive++;
				
				$plugin_count_installed++;
			}
		}
	}
	?>
	...
	<div class="table-filter">
		<a<?php echo $plugin_nav_class['all']; ?> href="<?php bb_uri( 'bb-admin/admin-base.php', array( 'plugin' => $_GET['plugin'] ), BB_URI_CONTEXT_A_HREF + BB_URI_CONTEXT_BB_ADMIN ); ?>"><?php printf( __( 'All <span class="count">(%d)</span>', 'plugin-browser-for-bbpress' ), $plugin_count_all ); ?></a> |
		<a<?php echo $plugin_nav_class['active']; ?> href="<?php bb_uri( 'bb-admin/admin-base.php', array( 'plugin_request' => 'active', 'plugin' => $_GET['plugin'] ), BB_URI_CONTEXT_A_HREF + BB_URI_CONTEXT_BB_ADMIN ); ?>"><?php printf( __( 'Active <span class="count">(%d)</span>', 'plugin-browser-for-bbpress' ), $plugin_count_active ); ?></a> |
		<a<?php echo $plugin_nav_class['inactive']; ?> href="<?php bb_uri( 'bb-admin/admin-base.php', array( 'plugin_request' => 'inactive', 'plugin' => $_GET['plugin'] ), BB_URI_CONTEXT_A_HREF + BB_URI_CONTEXT_BB_ADMIN ); ?>"><?php printf( __( 'Inactive <span class="count">(%d)</span>', 'plugin-browser-for-bbpress' ), $plugin_count_inactive ); ?></a> |
		<a<?php echo $plugin_nav_class['installed']; ?> href="<?php bb_uri( 'bb-admin/admin-base.php', array( 'plugin_request' => 'installed', 'plugin' => $_GET['plugin'] ), BB_URI_CONTEXT_A_HREF + BB_URI_CONTEXT_BB_ADMIN ); ?>"><?php printf( __( 'Installed <span class="count">(%d)</span>', 'plugin-browser-for-bbpress' ), $plugin_count_installed ); ?></a> |
		<a<?php echo $plugin_nav_class['updates']; ?> href="<?php bb_uri( 'bb-admin/admin-base.php', array( 'plugin_request' => 'updates', 'plugin' => $_GET['plugin'] ), BB_URI_CONTEXT_A_HREF + BB_URI_CONTEXT_BB_ADMIN ); ?>"><?php printf( __( 'Updates Available <span class="count">(%d)</span>', 'plugin-browser-for-bbpress' ), $plugin_count_updates ); ?></a>
	</div>
	...
	foreach ( $plugins as $plugin_folder => $plugin ) {
		if ( $plugin['version'] ) {
			...
			if ( $bb_plugins[$cpf] || $bb_plugins[$cpf2] ) {
				...
				if ( in_array( $cpf, array_values( $active_plugins ) ) ) {
					if ( !in_array( $plugin_request, array( 'active', 'installed', 'all', 'updates' ) ) ) {
						continue;
					}
					$class		=  'active';
					$action		= 'deactivate';
					...
					if ( !in_array( $plugin_request, array( 'inactive', 'installed', 'all', 'updates' ) ) ) {
						continue;
					}
					$class		= '';
					$action		= 'activate';
				...
				if ( version_compare( $plugin['version'], $bb_plugins[$cpf]['version'], '>' ) ) {
					if ( !in_array( $plugin_request, array( 'active', 'inactive', 'installed', 'all', 'updates' ) ) )
						continue;
				...
				} elseif ( $plugin_request == 'updates' ) {
					continue;
				}
			} elseif ( $plugin_request != 'all' ) {
				continue;
			}
