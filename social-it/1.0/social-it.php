...
function socialit_admin() {
	if($_GET['plugin'] == 'socialit_settings_page'){
		...
		if (detect7()) { //ie, as usual, doesnt render the css properly :| and creates problems for the developers
		...
		echo '<script type="text/javascript" src="'.SOCIALIT_PLUGPATH.'js/social-it.js"></script>';
	}
}
...
function socialit_insert_in_post($post_content) {
	...
	if (bb_is_topic() && $socialit_plugopts['topic'] == 1){ //socials should be generated and added
...
add_filter('post_text', 'socialit_insert_in_post');
?>