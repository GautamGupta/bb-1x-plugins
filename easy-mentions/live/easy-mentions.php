 ...
 * Defines
 */
define( 'EM_VER'	, '0.3-dev'						); /** Version */
define( 'EM_OPTIONS'	, 'EasyMentions'					); /** Option Name */
...
if ( !is_array( $em_plugopts ) ) { /* Get the options, if not found then set them */
	$em_plugopts = array(
		'link-tags'	=> 1,
		'link-users'	=> 1,
		...
		'add-nofollow'	=> 1,
		'reply-link'	=> 0,
		'reply-text'	=> '<em>' . sprintf( __( 'Replying to @%1$s\\\'s <a href="%2$s">post</a>:', 'easy-mentions' ), '%%USERNAME%%', '%%POSTLINK%%' ) . '</em>',
		'nav-to-last'	=> 1
	);
	bb_update_option( EM_OPTIONS, $em_plugopts );
}
if ( $em_plugopts['link-to'] ) { /* Update the old options of v0.1, will be removed in v0.4 */
