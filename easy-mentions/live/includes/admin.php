...
function em_options(){
	...
	if ( $_POST['em_opts_submit'] == 1 ) { /* Settings have been received, now save them! */
		...
		$em_plugopts['nav-to-last']	= ( intval( $_POST['nav-to-last']	) == 1 ) ? 1 : 0;
	...
	$em_options = array(
		...
		'link-user-to' => array(
			...
			'type'		=> 'radio',
			'class'		=> ( $em_plugopts['link-users'] != 1 ) ? array( 'disabled' ) : false,
		...
		'add-nofollow' => array(
			...
			'type'		=> 'checkbox',
			'class'		=> ( $em_plugopts['link-user-to'] != 'website' ) ? array( 'disabled' ) : false,
		...
		'reply-link' => array(
			...
			'note'		=> sprintf( __( 'Before checking this option, please verify that either there is a post form below the topic on each page (<a href="%s">help</a>) or the "Navigate to last page for reply" option is checked.', 'easy-mentions' ), 'http://bbpress.org/plugins/topic/easy-mentions/faq/' ),
			'options'	=> array(
				'1'	=> __( 'Yes', 'easy-mentions' )
			)
		),
		'reply-text' => array(
			...
			'class'		=> ( $em_plugopts['reply-link'] != 1 ) ? array( 'disabled' ) : false,
		...
		'nav-to-last' => array(
			'title'		=> __( 'Navigate to last page for reply?', 'easy-mentions' ),
			'type'		=> 'checkbox',
			'value'		=> ( $em_plugopts['nav-to-last'] == 1 ) ? '1' : '0',
			'class'		=> ( $em_plugopts['reply-link'] != 1 ) ? array( 'disabled' ) : false,
			'note'		=> __( 'Check this option if you don\'t want the reply form to be displayed on each page, and user should be navigated to the post form of last page of topic which he or she clicks the reply link.', 'easy-mentions' ),
			'options'	=> array(
				'1'	=> __( 'Yes', 'easy-mentions' )
			)
		)
	...
	if ( $em_plugopts['reply-link'] != 1 ) {
		...
		$em_options['nav-to-last']['attributes'] = array( 'disabled' => 'disabled' );
	}
	?>
