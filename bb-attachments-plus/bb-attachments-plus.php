<?php
/*
 Plugin Name: bbPress Attachments Plus
 Plugin URI: http://gaut.am/bbpress/plugins/bb-attachments-plus/
...
/**
 * @package bbPress Attachments Plus
 * @subpackage Main Section
 * @author Gautam Gupta (www.gaut.am)
 * @link http://gaut.am/bbpress/plugins/bb-attachments-plus/
 ...
 * Defines
 */
define( 'BBAP_VER'	, '0.1-dev'						); /** Version */
define( 'BBAP_OPTIONS'	, 'bbAttachments'					); /** Option Name */
define( 'BBAP_PLUGPATH'	, bb_get_plugin_uri( bb_plugin_basename( __FILE__ ) )	); /** Plugin URL */

/**
 * Options
 */
$bb_attachments = bb_get_option( BBAP_OPTIONS );
if ( !is_array( $bb_attachments ) ) { /* Get the options, if not found then set them */
	$bb_attachments = array(
		'role' => array( /* Minimum role to... (read/participate/moderate/administrate) */
			'see'		=> 'read',		/* See list of attachments */
			'inline'	=> 'read',		/* View inline reduced images */
			'download'	=> 'participate',	/* Download original */
			'upload'	=> 'participate',	/* Upload (times out with post edit time) */
			'delete'	=> 'moderate'		/* Delete */
		),
		'allowed' => array(
			'extensions' => array(
				'default'	=> array( 'gif', 'jpeg', 'jpg', 'pdf', 'png', 'txt', 'zip' ),				/* Anyone who can upload can submit these */
				'moderate'	=> array( 'gif', 'gz', 'jpeg', 'jpg', 'pdf', 'png', 'txt', 'zip' ),			/* Only if they can moderate */
				'administrate'	=> array( 'bmp', 'doc', 'gif', 'gz', 'jpeg', 'jpg', 'pdf', 'png', 'txt', 'xls', 'zip' )	/* Only if they can administrate */
			),
			'mime_type' => array(
				'default'	=> array( 'text/plain', 'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'application/pdf', 'application/x-pdf' ), /* For anyone that can upload */
				'moderate'	=> array( 'text/plain', 'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'application/pdf', 'application/x-pdf', 'application/zip', 'application/x-zip' , 'application/x-gzip' ),
				'administrate'	=> array( 'application/octet-stream', 'text/plain', 'text/x-c', 'image/bmp', 'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'application/pdf', 'application/x-pdf', 'application/zip', 'application/x-zip' , 'application/x-gzip' )
			)
		),
		'max' => array(
			'size' => array( /* In Bytes */
				'default'	=> 1024*100,	/* General max for all type/roles (100k) */
				'jpg'		=> 1024*150,	/* Size limit override by extension (200k) */
				'png'		=> 1024*150,	/* Size limit override by extension (200k) */
				'moderate'	=> 1024*200,	/* Size limit override by role (250k) - note this overrides all extension limits */
				'administrate'	=> 1024*500	/* Size limit override by role (500k) - note this overrides all extension limits */
			),
			'per_post' => array(
				'default'	=> 6,	/* How many files can be attached per post */
				'moderate'	=> 10	/* Override example, 'administrate' => 20 /* You don't even need to set for every role, this is just an example */
			),
			'uploads' => array(
				'default'	=> 6,	/* How many files can be uploaded at a time, in case you want to set per_post high */
				'moderate'	=> 10	/* Again, this is optional per extra roles */
			),
			'filename' => array(
				'default'	=> 40,	/* Maximum length of filename before auto-trim */
				'administrate'	=> 80	/* Override */
			),
			/* Advance setting */
			'php_upload_limit' => min( bb_attachments_l2n( ini_get( 'post_max_size' ) ), bb_attachments_l2n( ini_get( 'upload_max_filesize' ) ) ) /* Internal php upload limit - only edit if you know what you are doing */
		),
		'inline' => array(
			'width'		=> 590,		/* Max inline image width in pixels (for display, real width unlimited) */
			'height'	=> 590,		/* Max inline image height in pixels (for display, real height unlimited) */
			'solution'	=> 'resize',	/* resize|frame - Images can be either resized or CSS framed to meet above requirement, only resize is supported at this time */
			'auto'		=> true		/* Auto insert uploaded images into post */
		),
		'style' => '.bb_attachments_link, .bb_attachments_link img {border:0; text-decoration:none; background:none; } #thread .post li {clear:none; }',
		'aws' => array( /* The following is for Amazon S3 use, get key+secret here: https://aws-portal.amazon.com/gp/aws/developer/account/index.html#AccessKey */
			'enable'	=> false,					/* Amazon AWS S3 Simple Storage Service - http://amazon.com/s3 */
			'key'	=> '12345678901234567890',			/* Typically 20 letters+numbers */
			'secret'	=> '1234567890123456789012345678901234567890',	/* Must be exactly 40 characters long */
/** Advanced settings below */
			'bucket'	=> strtolower( 'bb-attachments.' . preg_replace( "/^(www?[0-9]*?\.)/i", '', $_SERVER['HTTP_HOST'] ) ),	/* Don't edit the following aws bucket or aws url unless you know what you are doing and have aws experience. If you rename the bucket, files are NOT moved automatically - you must do it manually via an S3 utility */
			/************************************************* Check **********************************************************/
			'url'	=> 'http://' . $bb_attachments['aws']['bucket'] . '.s3.amazonaws.com/'					/* Base url to amazon for retrieval, or may be a cname mirror off your own domain - http://docs.amazonwebservices.com/AmazonS3/2006-03-01/VirtualHosting.html#VirtualHostingCustomURLs . Cname example: bb-attachments.yoursite.com CNAME bb-attachments.yoursite.com.s3.amazonaws.com */
		),
		'icons' => array(
			'default'	=> 'default.gif',
			'bmp'	=> 'img.gif',
			...
			'xls'	=> 'xls.gif',
			'zip'	=> 'zip.gif',
			'url'	=> BBAP_PLUGPATH . 'icons/',
			'path'	=> rtrim( dirname( __FILE__ ), ' /\\' ) . '/icons/'
		),
		'path'		=> dirname( $_SERVER['DOCUMENT_ROOT'] ) . '/bb-attachments/', /* Make *NOT* WEB ACCESSABLE for security */
		'upload_on_new'	=> true, /* Allow uploads directly on new posts (set FALSE if incompatible for some reason) */
		/************************************************* Check **********************************************************/
		'title'		=> " <img class='bb_attachments_link' title='attachments' align='absmiddle' src='" . $bb_attachments['icons']['url'] . $bb_attachments['icons']['default']."' />", /* Text, html or image to show on topic titles if has attachments */
		'status'	=> array( 'OK', 'Deleted', 'Failed', 'Denied Extension', 'Denied Mime', 'Denied Size', 'Denied Count', 'Denied Duplicate', 'Denied Dimensions' ),
		'errors'	=> array( 'OK', 'Uploaded file exceeds UPLOAD_MAX_FILESIZE in php.ini', 'Uploaded file exceeds MAX_FILE_SIZE in the HTML form', 'Uploaded file was only partially uploaded', 'No file was uploaded', 'Temporary folder missing', 'Failed to write file to disk', 'File upload stopped by PHP extension' )
	);
	bb_update_option( BBAP_OPTIONS, $bb_attachments );
}
...
function bb_attachments_init() {
	...
	$bbdb->bb_attachments = $bbdb->prefix . 'attachments';

	bb_load_plugin_textdomain( 'bb-attachments-plus', dirname( __FILE__ ) . '/translations' ); /* Create Text Domain For Translations */

	if ( isset( $_GET['bb_attachments_diagnostic'] ) || isset( $_GET['bb_attachments_debug'] ) ) {
		include( 'debug.php' );
	} elseif ( isset( $_GET['bbat_delete'] ) ) {
		bb_attachments_delete();
	} elseif ( isset( $_GET['bb_attachments'] ) ) {
		if ( isset( $_GET['bbat'] ) ) {
	...
	} elseif ( isset( $_GET['new'] ) || bb_is_topic() || bb_is_forum() ) {
		...
		if ( bb_current_user_can( $bb_attachments['role']['upload'] ) ) {
			...
			if ( $bb_attachments['upload_on_new'] ) {
				...
				function bb_attachments_enctype() {
					global $topic, $forum;
					if ( ( bb_is_topic() && bb_current_user_can( 'write_post', $topic->topic_id ) ) || ( !bb_is_topic() && bb_current_user_can( 'write_topic', $forum->forum_id ) ) ) {
	...
	if ( $bb_attachments['title'] && !bb_is_topic() && !bb_is_feed() && !bb_is_admin() ) add_filter( 'topic_title', 'bb_attachments_title', 200 );
}

/**
 * Add the CSS in the header
 */
function bb_attachments_add_css() {
	global $bb_attachments;
	echo '<style type="text/css">' . $bb_attachments['style'] . '</style>';
}

function bb_attachments( $post_id = 0 ) {
	global $bb_attachments_on_page;
	if ( isset( $bb_attachments_on_page ) ) /* Only insert once per page -> pre 0.9.0.2 */
		return;
	else
...
function bb_attachments_post_attachments( $post_id = 0 ) {
	...
	if ( $post_id && ( $bb_attachments['role']['see'] == 'read' || bb_current_user_can( $bb_attachments['role']['see'] ) ) ) {
		...
		$self       = false;
		$admin      = false;
		$filter     = true; // " AND status = 0 "; // speedup checks with flag
		if ( $bb_current_user->ID == get_post_author_id( $post_id ) ) $self = true;
		if ( ( !bb_is_topic() || isset( $_GET['bb_attachments'] ) ) && bb_current_user_can( 'moderate' ) ) {
			$filter = '';
		...
		if ( !isset( $bb_attachments_cache[$post_id] ) )
			$bb_attachments_cache[$post_id] = $bbdb->get_results( "SELECT * FROM " . $bbdb->bb_attachments . " WHERE post_id = $post_id ORDER BY time DESC LIMIT 999" );
		...
		if ( $location=="edit.php") {
			...
			function bbat_inline_insert(post_id,id) {
				...
				if (document.selection) {
					bbat_field.focus(); sel = document.selection.createRange(); sel.text = bbat_value;
				} else if (bbat_field.selectionStart || bbat_field.selectionStart == "0") {
					var startPos = bbat_field.selectionStart; var endPos = bbat_field.selectionEnd; bbat_field.value = bbat_field.value.substring(0, startPos)+ bbat_value+ bbat_field.value.substring(endPos, bbat_field.value.length);
		...
		if ( preg_match_all( "/\[attachment=([0-9]+?)\,([0-9]+?)\]/sim", $text, $matches, PREG_SET_ORDER ) ) {
			foreach ( $matches as $match ) {
				...
				if ( file_exists( $fullpath ) ) { /* It's been resized, so it's likely on AWS, show directly */
					...
					if ( bb_is_feed() ) $replace = wp_specialchars( $replace );
...
function bb_attachments_process_post( $post_id = 0, $display = 0 ) {
	...
	$count		= intval( $bbdb->get_var( "SELECT COUNT(*) FROM " . $bbdb->bb_attachments . " WHERE post_id = $post_id AND status = 0" ) ); /* How many currently on post */
		...
		if ( !empty( $value ) ) {
			...
			if ( intval( $_FILES['bb_attachments']['error'][$key] ) == 0 && $_FILES['bb_attachments']['size'][$key] > 0) {
				...
				if ( $size > 0 && $filename ) { /* We still save the status code if any but don't copy file until status = 0 */
					$failed = $bbdb->get_var( "INSERT INTO " . $bbdb->bb_attachments . " ( `time`  , `post_id` , `user_id`, `user_ip`, `status` , `size` , `ext` , `mime` , `filename` )
					...
					if ( $status == 0 ) { /* Successful db insert - bbdb returns NULL on success so that !NULL is it's wierd way */
						...
						if ( !$failed && $id > 0 && file_exists( $tmp ) ) {
							...
							if ( $bb_attachments['aws']['enable'] )
								bb_attachments_aws( "$dir/", "$id.$filename", $mime ); // copy to S3
			...
			if ( $status > 0) {
				if ( $id > 0) $bbdb->query( "UPDATE " . $bbdb->bb_attachments . " SET `status` = $status WHERE `id` = $id" );
				$error = "";
				if ( $_FILES['bb_attachments']['error'][$key] > 0) $error = " (" . $bb_attachments['errors'][$_FILES['bb_attachments']['error'][$key]] . ") ";
...
function bb_attachments_upload_form( $post_id=0 ) {
	...
	if ( $post_id )
		...
		echo '<input  type="hidden" name="bb_attachments" value="0" />'; }
	...
	if ( $post_id )
		echo '<a style="margin-right:12em;" href="'. get_post_link( $post_id ) . '">' . __( "&laquo; return to post" ) . '</a>';
	else
		echo '<span style="margin-right:20em;">&nbsp;</span>';
...
function bb_attachments_download( $filenum = 0 ) {
	...
	if ( $filenum > 0 && ( $bb_attachments['role']['download'] == 'read' || bb_current_user_can( $bb_attachments['role']['download'] ) ) ) {
		$file = $bbdb->get_results( "SELECT * FROM " . $bbdb->bb_attachments . " WHERE id = $filenum AND status = 0 LIMIT 1" );
		if ( isset( $file[0] ) && $file[0]->id ) {
			...
			@$bbdb->query( "UPDATE " . $bbdb->bb_attachments . " SET downloads=downloads+1 WHERE id = $file->id LIMIT 1" );
			if ( $bb_attachments['aws']['enable'] ) {
				$aws = $bb_attachments['aws']['url'] . $file->id . '.' . $file->filename;
				header( 'Location: '.$aws );
			} elseif ( file_exists( $fullpath ) ) {
				if ( ini_get( 'zlib.output_compression' ) )
					ini_set( 'zlib.output_compression', 'Off' );	/* Fix for IE */
		...
		nocache_headers();
		echo "<html><body>" . __( 'Sorry, download is restricted.' ) . "<script type='text/javascript'>alert( '" . __( 'Sorry, download is restricted.' ) . "' );</script></body></html>";
	}
...
function bb_attachments_resize( $filename, $type, $width, $height, $suffix = ".resize" ) {
	...
	switch ( $type ) {
		case IMAGETYPE_JPEG:
			...
			$save= (string) 'imagejpeg';
			break;
		case IMAGETYPE_GIF:
			$img = imagecreatefromgif( $filename );
			$save = (string) 'imagegif';
			break;
		case IMAGETYPE_PNG:
			$img = imagecreatefrompng( $filename );
			$save = (string) 'imagepng';
			break;
		default:
			return false;
	}
	$scale_x = 1;
	$scale_y = 1;
	if ( $width > $bb_attachments['inline']['width'] ) $scale_x = $width/$bb_attachments['inline']['width'];
	if ( $height > $bb_attachments['inline']['height'] ) $scale_y = $height/$bb_attachments['inline']['height'];
	$scale = $scale_x;
...
function bb_attachments_delete( $filenum = 0 ) {
	...
	if ( $filenum > 0 && bb_current_user_can( $bb_attachments['role']['delete'] ) ) {
		...
		if ( isset( $file[0] ) && $file[0]->id && bb_current_user_can( 'edit_post', $file[0]->post_id ) ) {
			...
			if ( file_exists( $fullpath ) ) {
				@unlink( $fullpath );
				@$bbdb->query( "UPDATE " . $bbdb->bb_attachments . " SET status = 1 WHERE id = $file->id LIMIT 1" );
			}
...
function bb_attachments_recount( $post_id = 0 ) {
	...
	if ( $topic_id ) {
		$query = "SELECT count(status) as count FROM " . $bbdb->bb_attachments . " AS t1 LEFT JOIN $bbdb->posts AS t2 ON t1.post_id=t2.post_id
...
function bb_attachments_cache() {
	...
	if ( $posts ) {
		...
		$list = trim( $list, ' ,' );
		$results = $bbdb->get_results( "SELECT * FROM " . $bbdb->bb_attachments . " WHERE post_id IN ($list) ORDER BY id DESC" ); /* Needs to be optimized */
		if ( $results ) {
...
function bb_attachments_mime_type( $f ) {
	...
	if ( ( !$mime || strpos( $mime, 'file -bi' ) !== false ) && function_exists( 'getimagesize' ) && function_exists( 'image_type_to_mime_type' )  && strpos( $disabled, 'getimagesize' ) === false ) {
		...
		$mime = '';
		$imgt = @getimagesize( $f );
		$mime = image_type_to_mime_type( $imgt[2] ); // 0=width  1=height  if ( $imgt) {}
	}
...
function bb_attachments_install() {
	global $bbdb, $bb_attachments;
	$bbdb->bb_attachments = $bbdb->prefix."attachments";
	$bbdb->query( "CREATE TABLE IF NOT EXISTS " . $bbdb->bb_attachments . " (
	...
	$bbdb->query( "ALTER TABLE `" . $bbdb->bb_attachments . "` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci,
...
function bb_attachments_aws( $path, $name, $mime ) {
	...
	$fp = fsockopen( "s3.amazonaws.com", 80, $errno, $errstr, 15 );
	if ( !$fp ) return; // die("$errstr ( $errno)\n"); }
		...
		if ( strpos( $response, '<Error>' ) !== false) {
			...
			if ( strpos( $response, 'NoSuchBucket' ) !== false && $count==2 ) { /* Make bucket on bucket error */
				...
				$query = "PUT /{$bucket} HTTP/1.1\nHost: s3.amazonaws.com
					...
					Authorization: AWS {$key}:$hmac\n\n";
				$response = bb_attachments_aws_upload( $fp, $query );
				if ( strpos( $response, '<Error>' ) !== false) break; // echo $response; exit; }
			} /* Bucket error */
		} /* Upload error */
